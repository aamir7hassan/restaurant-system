...
class Drivers extends App_Controller
{ 
    ...
    public function test($show = 'All')
    {
        ...
        $this->render_page('driver/test', array('notices' => $notice,  'orders' => $orders, 'name' => $user->first_name));
    }

    public function index($show = "readyDriver")
    {
		//echo "<pre>";var_dump($this->config);die;
		if($show=="all") {
			$show = "driverall";
		} else if($show=="delivered") {
			$show = "driverdelivered";
		} else if($show == "ready") {
			$show = "readyDriver";
		} 
        ...
        if ($this->form_validation->run() !== FALSE) {
            if ($waiter_check) {
                ...
                if (isset($result['order_id']) && is_numeric($result['order_id']) > 0) {
                    ...
                    redirect('drivers/menu/order_' . $result['order_id'] . '/table_' . $result['table_id'], 'refresh');
                } else {
                    $this->session->set_flashdata('app_error', 'Unable to reserve location. Location is full.');
                    redirect(site_url('drivers/index'));
                }
            } else {
        ...
        $this->render_page('drivers/index', array('notices' => $notice,  'orders' => $orders, 'name' => $user->first_name, 'notifications' => $notifications, 'package' => $package, 'users' => $allusers,'user'=>$user));
    } /// end index
 
	public function map($show="all") {
		...
		if($show=="all") {
		 ...
		 $this->render_page('drivers/map', array('orders' => $orders));
	}
    ...
    public function get_ordering($show = 'All')
    {
        ...
        $this->render_page('drivers/index', array('notices' => $notice,  'orders' => $orders, 'name' => $user->first_name, 'notifications' => $notifications));
    }
    ...
    public function menu($order_id, $table_id)
    {
        ...
        $this->render_page('drivers/menu', array('meals' => $meals, 'order_id' => $order_id, 'order' => $data, 'table_id' => $table_id, 'display' => $display, 'show_avail_stock' => $show_avail_stock, 'hide_empty_stock' => $hide_empty_stock));
    }
    ...
    public function waiter($order_id, $table_id)
    {
        ...
        if ($this->input->post('waiter_code')) {
            ...
            if ($this->form_validation->run() !== FALSE) {
                ...
                $display = $this->customer_model->get_view_status();
                $this->render_page('drivers/new', array('meals' => $meals, 'order_id' => $order_id, 'order' => $data, 'table_id' => $table_id, 'display' => $display, 'show_avail_stock' => $show_avail_stock, 'hide_empty_stock' => $hide_empty_stock, 'order_details' => $order_details));
            } else {
                $this->session->set_flashdata('app_error', 'validation error');
                redirect($_SERVER['HTTP_REFERER']);
            }
        } else {
            $this->render_page('drivers/code');
        }
    }
    ...
    function print_item($ids)
    {
		...
		$user   = $this->ion_auth_model->user()->row();
		//echo "<pre>";var_dump($order_tot_price);die;
        $pdf->allow_charset_conversion=true;
        $pdf->charset_in='UTF-8';
        
        $pdf->autoLangToFont = true;
        $html = $this->load->view('drivers/slip', array('order' => $order, 'order_tot_price' => $order_tot_price, 'order_details' => $order_details,'user'=>$user), true);
    ...
    public function email_slip($ids,$email=null)
    {
        ...
        $pdfFilePath = $_SERVER["DOCUMENT_ROOT"]."/restaurant/application/attach/till_slip.pdf";
		$newFile = $_SERVER["DOCUMENT_ROOT"]."/restaurant/application/attach/".$name.".pdf";
    ...
    public function get_order()
    {
        ...
        if (!$user || !$this->ion_auth->is_waiter($user_id) || !$main_id) {
			...
			if($order_details->tip==null || empty($order_details->tip)) {
				$tip="";
			} else {
				$tip = round($order_details->tip);
			}
			//var_dump($order);die;
			
            echo json_encode(array("status" => 1, "order_id_error" => "", "order" => $order, "currency" => $currency, "vat" => $vat,'tip'=>$tip));
            //$this->output->set_content_type('application/json')->set_output(json_encode(array("status"=> 1, "order_id_error"=> "")));
            die;
        }
    }
    ...
    public function login()
    {
        ...
        $this->template->set_layout('waiter');
        $this->render_page('drivers/login', array());
    }
