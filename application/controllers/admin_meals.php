...
//                'attributes'                        => $attrs,
				...
				'show_available'					=> $meals->show_available
            );
        }
		// echo "<pre>";
	...
	public function show_available($meal_id="") {
        if(!is_numeric($meal_id))
            show_404 ();

        $this->meals_model->update_show_available($meal_id);
        redirect('admin/meals/available');
    }  
	
	public function cat_status($cat_id="") {
        if(!is_numeric($cat_id))
            show_404 ();

        $this->meals_model->cat_status($cat_id);
