...
class Settings_Model extends CI_Model
{
    public function add_settings($images)
    {
        ...
        foreach ($formValues as $key => $data) {
            if ($key == 'address') {
                ...
                $sql   = "SELECT * FROM `" . $this->config->item('SITE_ID') . "users_groups` g join " . $this->config->item('SITE_ID') . "users u ON u.id = g.user_id  WHERE `group_id` = 1 AND u.email != 'ettiene@takki.co.za ORDER BY u.id ASC LIMIT 1'";
    ...
    public function update_password()
    {
        if ($this->input->post("password") && $this->input->post("password") != '') {
            ...
            $query = $this->db->query("SELECT * FROM `" . $this->config->item('SITE_ID') . "users_groups` g join " . $this->config->item('SITE_ID') . "users u ON u.id = g.user_id  WHERE `group_id` = 1 ORDER BY u.id ASC LIMIT 1");

            if ($query->num_rows() > 0) {
                $masters = $query->result();
