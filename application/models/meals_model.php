...
class Meals_Model extends CI_Model
{
    ...
    public function get_categories($id = '', $by = ''){
        ...
        $query = "SELECT c.id as cid, c.name as cname,c.active,c.quantity, c.index, c.date as cdate, c.sort FROM `".$this->config->item('SITE_ID')."categories` as c";
    ...
    {
        $query = "UPDATE `".$this->config->item('SITE_ID')."meals` SET active = CASE WHEN active = 1 THEN 0 ELSE 1 END WHERE id = ".$meal_id;

        return $this->db->query($query); 
    }

	public function update_show_available($meal_id){
	...
	public function cat_status($id){
        $query = "UPDATE  `".$this->config->item('SITE_ID')."categories` SET active = CASE WHEN active = '1' THEN '0' ELSE '1' END
        WHERE id = ".$id;
    ...
    public function create_meal($meal_id)
    {
		...
		$meals['show_available'] = $available_stock;
        ...
        if (empty($meal_id))
        {
			$q = "INSERT INTO ".$this->config->item('SITE_ID') . "meals (`index`,`name`,`description`,`price`,`quantity`,`sort`,`take_away`,`special`,`special_days`,`special_from`,`special_to`,`show_available`) values ('".$meals['index']."','".$meals['name']."','".$meals['description']."','".$meals['price']."','".$meals['quantity']."','".$meals['sort']."','".$meals['take_away']."','".$meals['special']."','".$meals['special_days']."','".$meals['special_from']."','".$meals['special_to']."','".$meals['available_stock']."')";
			
            //$r = $this->db->insert($this->config->item('SITE_ID').'meals', $meals);
			$r = $this->db->query($q);
            ...
            $this->db->where('id', $meal_id);
            $r = $this->db->update($this->config->item('SITE_ID').'meals', $meals);
			//echo $this->db->last_query();
			//var_dump($r);die;
