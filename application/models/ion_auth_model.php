...
class Ion_auth_model extends CI_Model
{
	...
	public function __construct()
	{
		...
		$this->tables  = $this->config->item('tables', 'ion_auth');
                ...
                {
                    $this->tables[$key] = $this->config->item('SITE_ID').$table;
                }
		//initialize data
	...
	{
            $exist = $this->db->table_exists($this->config->item('SITE_ID').'users');
            if(!$exist)
                show_404 ();
	...
	public function login($identity, $password, $remember=FALSE, $facebook = FALSE)
	{
		...
		if (empty($identity) || (empty($password) && $facebook === FALSE))
		{
		...
		$query = $this->db->select($this->identity_column . ', phone, username, email, id, password, active, last_login')
		                  ->where($this->identity_column, $this->db->escape_str($identity))
                                  ->or_where('phone', $this->db->escape_str($identity))
                                  ->or_where('username', $this->db->escape_str($identity))
		                  ->get($this->tables['users']);
		...
		if ($query->num_rows() >= 1)
		{
			$users = $query->result();
                        foreach ($users as $user){ 
                            $password = $this->hash_password_db($user->id, $password_text);
	...
	public function user($id = NULL)
	{
		...
		$id || $id = $this->session->userdata('user_id');
                ...
                if($this->session->userdata('site_id') == $this->config->item('SITE_ID')){
                    $this->limit(1);
	...
	public function update($id, array $data)
	{
		...
		$this->trigger_events('extra_where');
		$r = $this->db->update($this->tables['users'], $data, array('id' => $user->id));
	...
	**/
	...
	protected function _prepare_ip($ip_address) {
        ...
        /**
         * @method captcha_insert insert captcha details to database
         * @param array $captcha Array with created captcha detaiLs
         */
        
        public function captcha_insert($captcha){
                $data = array(
